...
# Use `_nrg_queryBitBucket__depage.sh` list repositories available on the
# remote, printed to STDOUT, whose slugs match an ERE regex.
# Each line features the slug followed, optionally, by the link for SSH access,
# and/or the user-facing name, both delimited by tab characters.
#
...
# The slug must contain no space or tab characters and be fully lowercase, but
# the user-facing name may contain uppercase characters.
...
do
  case $OPT in
    ...
    r)  SHOW_SSH=1;;
    n)  SHOW_NAME=1;;
    ?)  printf "Usage: %s [-d] [-1] [-p <projectKey>] [-r] [-n] [<ERE>]\n" ${THIS}
        exit 2;;
  esac
done
...
# terminating on a tab character.
...
then
  ...
  #                    ^
  #                    |
  # This tab matches the final/terminating tab in RESLUG.
fi
if [ ! -z "${SHOW_NAME}" ]
then
